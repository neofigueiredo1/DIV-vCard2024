...
class conteudo_views extends HandleSql{
	...
	public function getView($nome=""){
		...
		if(file_exists('admin/modulos/conteudo/views/' . $nome . '.php')){
			require( $nome . '.php');
		}else{
	...
	public function get_page($pagina=0,$language='')
	{
		...
		$sql_pagina = self::select("Select tbPag.* From ".$this->DB_PREFIX."_conteudo_pagina as tbPag Where tbPag.status=1 And tbPag.pagina_idx=". (int)$pagina ." ");
		

		if($sql_pagina && is_array($sql_pagina))
		{
			$sql_pagina_mae = self::select("Select titulo,pagina_idx From ".$this->DB_PREFIX."_conteudo_pagina Where status=1 And pagina_idx=". (int)$sql_pagina[0]['pagina_mae'] ." ");
	...
	public function get_menufilho($pagina=0,$titulo="Mais informações")
	{
		...
		//$mysql_sys = new HandleSql;
		$sql_paginas = self::select("Select titulo,pagina_idx From ".$this->DB_PREFIX."_conteudo_pagina Where status=1 And pagina_mae=". (int)$pagina." Order By indice ASC, titulo ASC ");
	...
	public function get_menu($menu=0,$lista=false,$separador=false,$submenu=false,$inicio=false,$inicio_side='',$inicio_txt="In&iacute;cio", $language = '')
	{
		...
		if($sql_paginas && is_array($sql_paginas))
		{
			...
			$data_menu .= ($lista) ? '<ul id="lista-'.str_replace("_","-",$nome_menu).'" class="lista_'.$nome_menu.' lista_menu menu nav navbar-nav ml-0 mx-lg-3 mx-0" >' : '';
			// mx-3
			$counter=0;
			$counter_reg=0;
			...
			foreach($sql_paginas as $sql_pagina)
			{
				...
				$data_menu .= ($lista) ? '<li id="lista-item-'.$counter.'" slug_page="'.Text::friendlyUrl($sql_pagina['titulo']).'"  class="lista_item nav-item lista_item_'.$counter.' '.$first.$last.' odd" >' : '';

				$sql_pagina_mae = self::select("Select titulo From ".$this->DB_PREFIX."_conteudo_pagina Where status=1 And pagina_idx=".(int)$sql_pagina['pagina_mae']." ");
				...
				if($submenu){
					...
					$sql_paginas_filho = self::select("Select * From ".$this->DB_PREFIX."_conteudo_pagina Where status=1 And pagina_mae=" . (int)$sql_pagina['pagina_idx'] . " Order By indice ASC,titulo ASC ");

					if (count($sql_paginas_filho) > 0){

						$data_menu .= '<ul class="menu_dropdown dropdown-menu transition"><div class="wrapper wrapper-1120 columns">';
						...
						for ($i=0; $i < count($sql_paginas_filho); $i++) {
							...
							$data_menu .= '<li class="no_break_menu"><a class="dropdown-item bold" href="' . $url_fil . '" target="'.$url_fil_alvo.'" >' . $sql_paginas_filho[$i]['titulo' . $language_field] . '</a></li>';
						}
						$data_menu .= '</div></ul>';
