...
class banner_views extends HandleSql{
	...
	public function listByType($tipo=0)
	{
			$banners = parent::select("Select Banner.*,bTipo.nome as tipo_nome From ". $this->getPrefix() ."_banner as Banner
			    Inner Join ". $this->getPrefix() ."_banner_tipo as bTipo ON bTipo.tipo_idx = Banner.tipo_idx
			                          		Where Banner.status=1 And Banner.tipo_idx=".round($tipo)."
			                          		ORDER BY Banner.ranking DESC ");
			return $banners;
	}

	public function getBanner($tipo=0,$pagina=0,$randomize=0,$lista=0,$limite=0,$cycle=0,$cycle_pause=0,$arrayImages = 0,$prioridade=0,$onName=''){
		...
		if($tipo!=0)
		{
			...
			$whereOnName = (trim($onName)!='')?" And nome Like '".$onName."' ":"";

			//$mysql_sys = new manipula_sql;
			$banner_tipo = parent::select("Select * From ". $this->getPrefix() ."_banner_tipo Where tipo_idx=".round($tipo)." ");
			$banners = parent::select("Select Banner.* From ". $this->getPrefix() ."_banner as Banner Where Banner.status=1 And Banner.tipo_idx=".round($tipo)." " . $wherePagina . $whereOnName . " And (Banner.indica_data=0 Or (indica_data=1 And Banner.data_publicacao < now() And Banner.data_expiracao > now())) And (Banner.horario=0 Or Banner.horario=".round($horario).") ".$sql_orderby.$sql_limite);
	...
	public function getBannerData($tipo=0,$pagina=0,$randomize=0,$limite=0){
		...
		return parent::select("Select Banner.* From ". $this->getPrefix() ."_banner as Banner Where Banner.status=1 And Banner.tipo_idx=".round($tipo)." " . $wherePagina . " And (Banner.indica_data=0 Or (indica_data=1 And Banner.data_publicacao < now() And Banner.data_expiracao > now())) And (Banner.horario=0 Or Banner.horario=".round($horario).") ".$sql_orderby.$sql_limite);
